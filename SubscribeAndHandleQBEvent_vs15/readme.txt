...
SmartCalc.exe  -u <Menu item Name>
...
Running the sample:
------------------------
Before running SmartCalc.exe, this exe need to be registered as COM out of proc server. 
Run SmartCalc.exe -h to see the various options with the exe.
...
For Data Event subscription:
	- To change the code to subscribe to events other than customer events, modify GetDataEventSubscriptionAddXML function in SmartCalcs.cs. The xml can be modified to subscribe to any other list (other then customer) event and any action on the list.

For UI Extension subscription:
	- To add the menu item under any other QB menu modify the xml in GetUIExtensionSubscriptionAddXML function in SmartCalcs.cs. Refer to QB XML on screen reference for more details on qbXML.
