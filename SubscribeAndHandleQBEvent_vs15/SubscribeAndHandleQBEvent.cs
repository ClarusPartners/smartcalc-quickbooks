...
namespace SmartCalc
{
    ...
    // Note that SmartCalc is NOT declared as public.
    ...
    class SmartCalc
    {
        ...
        private static async Task ProcessEvents(CancellationToken cancellationToken)
        {
            ...
            {
                if (eventQueue.TryDequeue(out SmartCalcEvent currentEvent))
                {
                    var smartCalc = new SmartCalcHandler();

                    var invoice = SmartCalcHandler.QueryInvoice(currentEvent.RefNumber);

                    decimal taxRate = await smartCalc.GetSalesTaxFromSmartCalcAsync(invoice);
                ...
                {
                    // No events in the queue, wait a little before checking again
        ...
        private static string GetDataEventSubscriptionAddXML()
        {
            ...
            txnEventSubscription.AppendChild(requestXMLDoc.CreateElement("TxnEventType")).InnerText = "Invoice";
        ...
        public static void EnqueueAllLinesAsEvents(string filePath)
        {
            //Console.WriteLine($"EnqueAllLinesAsEvents() Method entered");
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(fileStream))
                {
                    ...
                    while ((line = reader.ReadLine()) != null)
                    {
                        ...
                        {
                            // Enqueue the line as an event
                            SmartCalcEvent newEvent = new SmartCalcEvent { RefNumber = line };
                            eventQueue.Enqueue(newEvent);
                            //Console.WriteLine($"Enqueued event: {newEvent.RefNumber}");
                        }
                    }
            ...
            {
                //Console.WriteLine($"Unexpected error reading from file: {ex.Message}");
            }
        }
        ...
        static void Main(string[] args)
        {
                ...
                try
                {
                    ...
                    //Console.WriteLine(factory.ClassId);
                    ...
                    //Console.WriteLine("Waiting for an Invoice to be generated .....\n");
                   //Task.Run(() => ProcessEvents(cancellationTokenSource.Token));

                    // Start the message loop.
                    MSG msg;
                    IntPtr null_hwnd = new IntPtr(0);
                    // //Console.WriteLine( GetMessage(out msg, null_hwnd, 0, 0));
                    while (GetMessage(out msg, null_hwnd, 0, 0) != false)
                    {

                        TranslateMessage(ref msg);
                        DispatchMessage(ref msg);
                       // EnqueueAllLinesAsEvents(Path.Combine(Path.GetTempPath(), $"QBGeneratedInvoices.txt"));
                    }
