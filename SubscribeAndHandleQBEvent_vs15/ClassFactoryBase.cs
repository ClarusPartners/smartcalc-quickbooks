...
namespace SmartCalc
{
	class ClassFactoryBase : IClassFactory
	{
		...
		{
			//Console.WriteLine("ClassFactoryBase Constructor");
		}
		...
		public void LockServer(bool bLock)
		{
			if (bLock)
			{
                SmartCalc.InterlockedIncrementServerLockCount();
			}
			else
			{
                SmartCalc.InterlockedDecrementServerLockCount();
			}
